...
if(posts)
  for (var i = 0; i < post_data.length; i++) {
    ...
    <div class="info">
      ...
      <div class="more">
          <img src="http://127.0.0.1:8000/static/images/show_more.png" alt="show more">
      </div>
    </div>
    <div class="image">
...
<!-- Preload the images to prevent delays -->
<link rel="preload" as="image" href="/static/images/love.png">
<link rel="preload" as="image" href="/static/images/heart.png">

<!-- Like button -->
<div class="like">
  <img
    class="like-toggle"
    data-post-id="${post_data[i][7]}"
    src="/static/images/love.png"
    data-liked-src="/static/images/heart.png"
    data-unliked-src="/static/images/love.png"
    data-is-liked="false"
  ...
  />
      ...
      <div class="post_desc">
          ...
          <p><a class="gray commants" data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="${post_data[i][7]}" href="#">View all ${post_data[i][6]} comments</a></p>
          <form action="/home/${post_data[i][7]}" method="post">${post_data[i][8]}
        <form>
  <input type="text" name="comment" id="emoji_comment" placeholder="Add a comment..." />
  <input type="submit" class="text text-info" value="Post" style="background: #ffffff;">
</form>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const preloadImages = ["/static/images/love.png", "/static/images/heart.png"];
    ...
    likeButtons.forEach(button => {
      button.addEventListener("click", function () {
        ...
        const isLiked = img.dataset.isLiked === "true";
        const postContainer = img.closest(".desc");
  
        // Instant feedback (optimistic update)
        img.src = isLiked ? img.dataset.unlikedSrc : img.dataset.likedSrc;
        img.dataset.isLiked = (!isLiked).toString();
        img.title = !isLiked ? "Undo like" : "Like this post";
  
        // Optimistically update like count
        const likeCountElem = postContainer?.querySelector(".liked a");
        if (likeCountElem) {
          const count = parseInt(likeCountElem.textContent) || 0;
          likeCountElem.textContent = `${isLiked ? count - 1 : count + 1} likes`;
        }
        ...
        fetch(`/like-post/${postId}/`, {
          ...
          .then(data => {
            if (data.success) {
              img.src = data.liked ? img.dataset.likedSrc : img.dataset.unlikedSrc;
              img.dataset.isLiked = data.liked.toString();
              img.title = data.liked ? "Undo like" : "Like this post";
  
              if (likeCountElem) {
                likeCountElem.textContent = `${data.likes} likes`;
              }
            }
          })
  ...
  <div class="item">
      ...
      <div class="bg"
        <div class="info">
          ...
          <div class="comments">
              <img src="./images/message.png" >dsdsd
              <span>${explore_date[i++][2]}</span>
          </div>
        </div>
      </div>
